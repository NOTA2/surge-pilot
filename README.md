...
macOS Finder에서 [SurgePilot.command](SurgePilot.command)을 더블클릭하면 로컬 웹앱이 열립니다. 터미널에서 시작하려면 `./run-web.sh`를 실행하세요. 주소는 `http://127.0.0.1:8765/`입니다. 서버가 실행되는 동안 브라우저에서 N/M/X/Y를 바꿔 **분석 실행**을 누르면 완료 후 상세 HTML이 같은 화면에 갱신됩니다. **현재 외부 IP 확인**과 **장전 분봉 수집 + 보고서 갱신**도 화면에서 실행할 수 있습니다. 종료하려면 열린 터미널에서 `Ctrl+C`를 누르세요.
