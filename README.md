...
- JSON 보고서를 GitHub Pages에서 열어 거래 내역, 신호, 평가금액과 검증 정보를 볼 수 있습니다. 장중 화면은 지원되는 브라우저에서 로컬 보고서 파일을 연결하면 15초마다 최신 내용을 다시 읽습니다.
...
같은 컴퓨터의 Chrome 또는 Edge에서 [Pages 장중 화면](https://nota2.github.io/surge-pilot/live.html)을 열고 **로컬 파일 연결**로 `reports/private/paper.json`을 선택하면 15초마다 바뀐 파일을 다시 읽습니다. 파일 내용은 GitHub에 업로드하지 않습니다. 파일 자동 갱신 API가 없는 브라우저에서는 **보고서 1회 열기**를 사용하세요. 이 연결은 **가상매매 테스트용**이며, 컴퓨터를 끄면 끊깁니다.
