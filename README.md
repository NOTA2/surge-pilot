...
포착과 진입도 따로 평가합니다. 첫 +20% 신호 직후 연속된 다음 분봉 시가를 진입가로 가정하고 30분 동안 `+10% 목표`와 `-5% 손절`의 선후를 본 결과, 50% 급등 사례 262건 중 목표가 먼저인 경우는 88건, 손절이 먼저인 경우는 149건이었습니다(같은 봉에서 둘 다 닿은 8건은 순서 미확정). 급등주를 맞혀도 바로 추격 매수하면 흔들림에 먼저 밀릴 수 있다는 탐색 결과입니다. 분봉의 호가 간격·체결 실패·수수료는 반영하지 않았습니다. 첫 +20% 경보 시점의 최근 5분 거래대금 10만 달러를 필수 조건으로 걸면 급등주 294건 중 141건만 남으므로, 거래대금은 포착 단계의 고정 하한보다 우선순위와 진입 가능 수량 판단에 사용해야 합니다.

같은 저장 데이터로 두 진입 가설도 비교했습니다. `5% 눌림 후 2분 연속 회복`은 급등주 사전 진입 137건, `3분 관찰 후 종가 고점 1% 돌파`는 112건으로, 즉시 진입의 262건보다 포착 가능한 기회가 줄었습니다. 급등주에서 두 방식의 전체 진입 결과는 각각 목표 선도달 59건·손절 선도달 128건, 59건·96건이었습니다. **동일한 종목·날짜**에 즉시 진입했다고 가정하면 각각 70건·124건, 69건·84건이므로 이 표본에서 두 방식의 개선 근거는 없습니다. 20~49%에서 멈춘 대조군도 함께 보고서에 표시합니다. 이는 30거래일의 탐색이며 전체 시장 후보군·실제 체결·별도 기간에서 검증되지 않았으므로 자동 매수 규칙으로 사용하지 않습니다.
